...
import { dispatchPlay, dispatchLoadbg, computeDurationMsFromTrim } from '../lib/playoutDispatch';
...
let onAdvanceCallback = null;
let playToken = 0;
let consecutiveSkips = 0;
const MAX_CONSECUTIVE_SKIPS = 3;
let advanceInFlight = false;
let feedbackListenerPromise = null;
let feedbackUnlisten = null;
let tickUnlisten = null;
let advanceUnlisten = null;
let reconnectTimer = null;
let reconnectAttempt = 0;
let reconnectRequested = false;
let reconnectInFlight = null;
let heartbeatTimer = null;
const assertIngestorReady = (item) => {
    const status = item.ingestorStatus || 'idle';
    if (status !== 'ready' && status !== 'idle') {
        throw new Error(`Cannot play item "${item.filename}" — Ingestor status is "${status}". Asset must be "ready" to play.\n` +
            `UUID: ${item.playoutvueId || 'N/A'}\n` +
...
const startHeartbeat = () => {
    ...
    heartbeatTimer = setInterval(() => {
        if (!isCasparConnected.value || reconnectInFlight)
            return;
...
const prepareCasparMediaPath = async (rawPath) => {
    if (!rawPath)
        return '';
    try {
        return await invoke('prepare_caspar_media_path', {
            path: rawPath,
...
const resolveLogoAsset = (filename) => {
...
const getRatingAssetPath = (rating) => {
...
const itemDurationMs = (item) => {
    ...
    const totalMs = item.duration_ms || item.durationMs || (item.duration ? item.duration * 1000 : 0);
...
const stripMediaExtension = (value) => value.replace(/\.[^./\\]+$/, '');
const parseCasparTimecodeMs = (value, fps = PAL_FPS) => {
...
const parseNumericXmlTag = (response, tagName) => {
...
const parseDurationFromCasparResponse = (response) => {
    if (!response)
        return 0;
    ...
    const frameCount = parseNumericXmlTag(response, 'file-nb-frames') ||
        parseNumericXmlTag(response, 'nb-frames') ||
        parseNumericXmlTag(response, 'frame-count');
    if (frameCount > 0) {
        const fps = parseNumericXmlTag(response, 'fps') ||
...
const parseDurationFromCasparList = (response, clipKey) => {
...
const queryCasparDurationMs = async (item) => {
...
const updateItemDurationFromMs = (item, durationMs) => {
    if (durationMs <= 0)
        return 0;
...
const ensureItemDurationMs = async (item) => {
    ...
    try {
        const metadata = await invoke('scan_media', { filepath: scanPath });
...
const waitForDurationResolution = async (item, timeoutMs) => {
...
async function refreshCurrentProducerDuration(item, key, token) {
    for (let attempt = 0; attempt < 6; attempt += 1) {
        if (!isCasparPlaying.value || token !== playToken)
            return;
...
const buildLiveCommand = (preferredSource) => {
...
const sendRawCommandCore = async (cmd) => {
    return invoke('caspar_send_command', { cmd });
};
...
const ensureFeedbackListener = async () => {
    ...
    feedbackListenerPromise = (async () => {
        ...
        if (!tickUnlisten) {
            tickUnlisten = await listen('caspar://playback-tick', (event) => {
        ...
        if (!advanceUnlisten) {
            advanceUnlisten = await listen('caspar://advance', (event) => {
...
const runReconnectAttempt = async (foreground) => {
        ...
        for (let attempt = 0; attempt < attempts; attempt += 1) {
            try {
                ...
                if (foreground && attempt < attempts - 1) {
                    const delay = Math.min(RECONNECT_BASE_DELAY_MS * 2 ** attempt + jitter(), RECONNECT_MAX_DELAY_MS);
...
function scheduleReconnect() {
    if (!reconnectRequested || reconnectTimer || reconnectInFlight)
        return;
...
const sendRawCommand = async (cmd) => {
    try {
        ...
        const isTransportError = /timed out|connect|econnreset|econnrefused|broken pipe|connection refused/i.test(message);
        if (isTransportError) {
...
async function preloadNextItemAt(index, retriesLeft = 6, delayMs = 500) {
    if (index < 0 || index >= queuedItems.length)
        return;
    const item = queuedItems[index];
    if (!item || item.type !== 'video' || item.ingestorStatus === 'error')
        return;
...
async function playItemAt(index, token) {
    try {
        const item = queuedItems[index];
        ...
        if (nextItem && nextItem.type === 'video') {
            ...
            try {
                ...
                nextPath = await invoke('prepare_caspar_media_path', {
                    path: nextRawPath,
        ...
        const dispatchResult = await dispatchPlay(hydrated, PROGRAM_CHANNEL, CASPAR_LAYERS.video, nextPath);
        isCasparPlaying.value = true;
        consecutiveSkips = 0;
        ...
        setTimeout(() => {
            if (token !== playToken)
                return;
            refreshCurrentProducerDuration(item, key, token).catch((error) => {
...
async function advanceToNext(token, natural) {
    if (token !== playToken)
        return;
    if (advanceInFlight)
        return;
    ...
    try {
        ...
        const isNaturalVideoTransition = natural &&
            currentItem &&
        ...
        if (isNaturalVideoTransition && nextItem) {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    refreshCurrentProducerDuration(nextItem, key, currentPlayToken).catch((error) => {
                        console.warn('[CasparCG] Failed to refresh active producer duration', error);
                    });
                }, 250);
            }
...
export const casparPlayoutService = {
    engine: 'casparcg',
    label: 'CASPAR',
    supports: {
        preview: false,
    ...
    async syncBrandingAssets() {
        ...
        if (logoEnabled && logoPath) {
            await invoke('caspar_play_image', { channel: PROGRAM_CHANNEL, layer: logoLayer, path: logoPath }).catch((e) => {
    ...
    async seekMedia(_inputName, timeCursor) {
    ...
    async applyComplianceForItem(item) {
        ...
        timelineTimers.forEach(clearTimeout);
        timelineTimers = [];
        const rating = (item.complianceRating || 'none');
        const tpFlag = !!item.tp_flag;
        // Age rating badge (image producer via typed command).
        let ratingSourcePath = '';
        if (rating === 'k')
            ratingSourcePath = settings.cgRatingKPath;
        else if (rating === '8')
            ratingSourcePath = settings.cgRating8Path;
        else if (rating === '12')
            ratingSourcePath = settings.cgRating12Path;
        else if (rating === '16')
            ratingSourcePath = settings.cgRating16Path;
        else if (rating === '18')
            ratingSourcePath = settings.cgRating18Path;
        ...
        if (ratingSourcePath) {
            ...
            await invoke('caspar_play_image', { channel: PROGRAM_CHANNEL, layer: ratingLayer, path }).catch((e) => {
        ...
        if (tpFlag && tpSourcePath) {
            ...
            await invoke('caspar_play_image', { channel: PROGRAM_CHANNEL, layer: tpLayer, path }).catch((e) => {
    ...
    async startDeckLink(outputName) {
        if (!isCasparConnected.value)
            await this.connect();
        ...
        if (settings.decklinkEmbeddedAudio)
            cmdParts.push('EMBEDDED_AUDIO');
        if (settings.decklinkLatency && settings.decklinkLatency !== 'normal')
            cmdParts.push(`LATENCY_${settings.decklinkLatency.toUpperCase()}`);
        if (settings.decklinkKeyer && settings.decklinkKeyer !== 'external')
            cmdParts.push(`KEYER_${settings.decklinkKeyer.toUpperCase()}`);
        if (settings.decklinkBufferDepth && settings.decklinkBufferDepth !== 3)
            cmdParts.push(`BUFFER_DEPTH ${settings.decklinkBufferDepth}`);
        if (settings.decklinkKeyDevice && settings.decklinkKeyDevice > 0)
            cmdParts.push(`KEY_DEVICE ${settings.decklinkKeyDevice}`);
        await sendRawCommand(`REMOVE ${PROGRAM_CHANNEL} DECKLINK ${deviceId}`);
        await sendRawCommand(cmdParts.join(' '));
    },
    async stopDeckLink(outputName) {
        if (!isCasparConnected.value)
            await this.connect();
        const deviceMatch = outputName.match(/\d+/);
        const deviceId = deviceMatch ? deviceMatch[0] : '1';
